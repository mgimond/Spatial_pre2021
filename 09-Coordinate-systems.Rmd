...
ext <- tmaptools::bb(matrix(c(-90, 25, 35,60)), projection="+proj=longlat +datum=WGS84")
...
gc <- gcIntermediate(Waterville, Paris)
...
pl_sf <- data.frame(rbind(Waterville, Paris)) %>%   
             mutate(id =1) %>% 
...
wrld  <- World %>%  mutate(id=1) %>%  group_by(id) %>% summarize() %>% 
  raster::rasterize(.,y=r, field="id")
...
tm_shape(wrld, bbox=ext) + 
  ...
  tm_grid(projection="longlat", col="grey90", labels.size = 0) +
...
tm_shape(wrld2) +
  tm_raster(legend.show = FALSE, palette= c("grey80")) +
  tm_grid(n.x=15,projection="longlat", col="grey90", labels.size = 0) +
  tm_shape(gc_sf) +tm_lines(col="red", lwd=2) +
  tm_shape(pl_sf) + tm_lines(col="blue", lwd=2, lty=2) +
  tm_shape(pt) +tm_dots(size=0.5) 
```
