...
```{r f11-quad02, fig.cap="Example of a covariate. Figure on the left shows the elevation map. Figure on the right shows elevation broken down into four sub-regions for which a local density will be computed.", fig.height=2, fig.width=6.0, echo=FALSE}
...
```{r f11-quad03, fig.cap="Figure on the left displays the number of points in each elevation sub-region (sub-regions are coded as values ranging from 1 to 4). Figure on the right shows the density of points (number of points divided by area of sub-region).", fig.height=2, fig.width=6.0, echo=FALSE}
...
```{r f11-kernel03, fig.cap="An estimate of $\\rho$ using the ratio method. The figure on the left shows the estimated $\\rho$ as a function of elevation. The envelope shows the 95% confidence interval. The figure on the right shows the spatial distribution of $\\rho$.", fig.height=2, fig.width=5, echo=FALSE}
...
```{block, type="note"}
Note: The left-hand side of a logisitic regression model is often presented as the *probability*, $P$, of occurance and is related to $\lambda$ as \$\\lambda=P/(1-P)\$ which is the *ratio of probability of occurence*. Solving for $P$ gives us \$P = \\lambda/(1 + \\lambda)\$ which yields the following equation:
$$
...
```{r f11-ppp-dist, fig.cap="Distance between each point and its closest point. For example, the point closest to point 1 is point 9 which is 2.32 units away.", results="asis", echo=FALSE, message=FALSE, warning=FALSE, fig.height=3.0, fig.width=5}
...
```{r f11-ANN-plot,fig.cap="ANN values for different neighbor order numbers. For example, the ANN for the first closest neighbor is 1.52 units; the ANN for the 2nd closest neighbor is 2.14  units; and so forth.", fig.height=1.2, fig.width=2.5, echo=FALSE, message=FALSE, warning=FALSE}
